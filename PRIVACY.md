...
The extension uses Chrome's local storage (`chrome.storage.local`) to remember four settings between sessions:
...
| `siteModes` | hostnames mapped to renderer choices | Sites you've switched to Beta mode. |
...
- **`storage`** — to save your settings (the four keys above) on your device.
